...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <h5 class="mb-0 h6">{{translate('Edit Cell')}}</h5>
</div>
<div class="">
    <form class="form form-horizontal mar-top" action="{{route('cell.update', $cell->id)}}" method="POST" enctype="multipart/form-data" id="choice_form">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <input type="text" class="form-control" name="name" value="{{ $cell->name }}" placeholder="Cell Name" required>
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <input type="text" class="form-control" name="code" value="{{ $cell->code }}" placeholder="Cell Code" required>
                            </div>
                        </div>
                            ...
                            <div class="col-md-8">
                                <select class="form-control aiz-selectpicker" name="warehouse_id" placeholder="Select Warehouse" data-live-search="true" required>
                                    @foreach ($warehouses as $key => $warehouse)
                                        <option
                                            {{ $cell->warehouse_id == $warehouse->id ? "selected" : "" }}
                                            value="{{ $warehouse->id  }}"
                                        >
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <select class="form-control aiz-selectpicker" name="location_id" placeholder="Select Location" data-live-search="true" required>
                                    @foreach($cell->shelf->area->location->warehouse->locations as $location)
                                        <option
                                            {{ $cell->location_id == $location->id ? "selected" : "" }}
                                            value="{{ $location->id }}"
                                        >
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <select class="form-control aiz-selectpicker" name="area_id" placeholder="Select Area" data-live-search="true" required>
                                    @foreach($cell->shelf->area->location->areas as $area)
                                        <option
                                            {{ $cell->area_id == $area->id ? "selected" : "" }}
                                            value="{{ $area->id }}"
                                        >
                                            {{ $area->name . ' (' . $area->code . ')' }}
                                        </option>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <select class="form-control aiz-selectpicker" name="shelf_id" placeholder="Select Shelf" data-live-search="true" required>
                                    @foreach($cell->shelf->area->shelves as $shelf)
                                        <option
                                            {{ $cell->shelf_id == $shelf->id ? "selected" : "" }}
                                            value="{{ $shelf->id }}"
                                        >
...
<script type="text/javascript">
    ...
    $("select[name='location_id']").on('change', function() {
        $.ajax({
            type: "GET",
            url: "{{ route('shelf.get-areas-by-location') }}",
            data: {
                _token: "{{ csrf_token() }}",
    ...
    $("select[name='area_id']").on('change', function() {
        $.ajax({
            type: "GET",
            url: "{{ route('cell.get-shelves-by-area') }}",
            data: {
                _token: "{{ csrf_token() }}",
