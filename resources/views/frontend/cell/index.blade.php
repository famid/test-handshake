    ...
    <div class="aiz-titlebar text-left mt-2 mb-3">
        <div class="row align-items-center">
            ...
            <div class="col-md-6 text-md-right">
                <a href="{{route('seller.cell.create')}}" class="btn btn-primary">
            ...
            <table class="table aiz-table mb-0">
                ...
                @foreach ($cells as $key => $cell)
                    <tr>
                        ...
                        <td>{{ $cell->location->name ?? "N/A"}}</td>
                        <td>{{ $cell->area->name ?? "N/A" }}</td>
                        <td>{{ $cell->shelf->name ?? "N/A" }}</td>

                        <td class="text-right">
                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                               href="{{ route('seller.cell.edit', $cell->id) }}"
                               title="{{ translate('Edit') }}">
                                <i class="las la-edit"></i>
                            </a>

                            <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete"
                               data-href="{{ route('seller.cell.destroy', $cell->id) }}"
