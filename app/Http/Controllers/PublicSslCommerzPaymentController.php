...
//                    $post_data['cus_email'] = $request->session()->get('shipping_info')['email'];
                ...
                elseif (Session::get('payment_type') == 'customer_package_payment') {
                    $customer_package = Package::findOrFail(Session::get('payment_data')['customer_package_id']);
                    $user_info = [
                    ...
                    $post_data = array();
                    $post_data['total_amount'] = $request->session()->get('payment_data')['package_price']; # You cant not pay less than 10
                    $post_data['currency'] = "BDT";
                    $post_data['tran_id'] = substr(md5(Auth::user()->id), 0, 10); // tran_id must be unique
                    ...
                    $post_data['value_c'] = $request->session()->get('payment_type');
                    $post_data['value_d'] = implode(',',$user_info);
                ...
                elseif (Session::get('payment_type') == 'customer_package_renew_payment') {
                    $customer_package = checkPackage();
                    $user_info = [
                        'user_id' => Auth::user()->id,
                        'user_type' => loginType()
                    ];
                    ...
                    $post_data = array();
                    $post_data['total_amount'] = $customer_package->Package->package_price; # You cant not pay less than 10
                    $post_data['currency'] = "BDT";
                    $post_data['tran_id'] = substr(md5(Auth::user()->id), 0, 10); // tran_id must be unique

                    $post_data['value_a'] = $post_data['tran_id'];
                    $post_data['value_b'] = $customer_package->id;
                    $post_data['value_c'] = $request->session()->get('payment_type');
                    $post_data['value_d'] = implode(',',$user_info);
    ...
    public function success(Request $request)
    {
        ...
        if(isset($request->value_c)){
            ...
            elseif ($request->value_c == 'customer_package_payment') {
                ...
                $data['payment_method'] = 'sslcommerz';
                $user = explode(',',$request->value_d);

                Auth::login(User::find($user[0]));
                Session::put('login_as',$user[1]);

                $customer_package_controller = new CustomerPackageController;
                return $customer_package_controller->purchase_payment_done($data, $payment);
            }
            elseif ($request->value_c == 'customer_package_renew_payment') {
                $data['customer_package_id'] = $request->value_b;
                $data['payment_method'] = 'sslcommerz';
                $user = explode(',',$request->value_d);
                ...
                $customer_package_controller = new CustomerPackageController;
                return $customer_package_controller->renew_package_payment_done($data, $payment);
            }
