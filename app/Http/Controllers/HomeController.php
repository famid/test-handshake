...
use App\Models\DarazIntregation;
...
class HomeController extends Controller
{
    ...
    public function login()
    {
        ...
        $vendor_login = false;
        return view('frontend.user_login',compact('vendor_login'));
    }
    ...
    public function dashboard()
    {
        ...
        elseif($login_type == 'seller'){

            if (session()->get('daraz_active_shop') != null) {

                $shops = DarazShop::where('user_id',Auth::user()->id)->get();

                return view('frontend.user.customer.dashboard',compact('shops'));
            }else{
    ...
    public function sellerCreate()
    {
        ...
        if ($user->user_type == 'seller' && $user->has_multi_account == 0) {

            $user->has_multi_account = 1;
            $user->save();
        ...
        return view('frontend.seller_confirm');
    }

    public function switchAccout()
    {

        if (loginType() == 'seller') {

            Session::forget('login_as');
            Session::put('login_as', 'vendor');
            flash(translate('Login as vendor succesfully'))->success();
            return redirect()->route('dashboard');

        }else{

            Session::forget('login_as');
            Session::put('login_as', 'seller');
            flash(translate('Login as seller succesfully'))->success();
            return redirect()->route('dashboard');
        }
    }
    ...
    public function profile(Request $request)
    {
        if(Auth::user()->user_type == 'delivery_boy'){
            ...
            $daraz_setting = DarazIntregation::where('user_id',Auth::user()->id)->first();

            return view('frontend.user.profile',compact('daraz_setting'));
        }
    }
...
//        $categories = Category::where('level', 0)->orderBy('name', 'asc')->get();
    ...
    {
        $calculate = (500 / 30) * (30 - 2);

        $customer_packages = Package::where('package_type',loginType())->get();
