...
Route::group(['prefix' => 'seller', 'middleware' => ['seller', 'verified', 'user']], function() {
    ...
    Route::as('seller.')->group(function () {
        ...
        Route::get('/area/destroy/{id}', 'AreaController@destroy')->name('area.destroy');
        Route::get('/shelf/destroy/{id}', 'ShelfController@destroy')->name('shelf.destroy');
        Route::get('/cell/destroy/{id}', 'CellController@destroy')->name('cell.destroy');

        Route::get('/get-locations-by-warehouse', 'AreaController@getLocationsByWarehouse')->name('area.get-locations-by-warehouse');
        Route::get('/get-areas-by-location', 'ShelfController@getAreasByLocation')->name('shelf.get-areas-by-location');
        Route::get('/get-shelves-by-area', 'CellController@getShelvesByArea')->name('cell.get-shelves-by-area');
        Route::get('/get-cells-by-shelf', 'CellController@getCellsByShelf')->name('cell.get-cells-by-shelf');
    });

});

//Route::middleware(['auth', 'seller', 'verified', 'user'])->prefix('/seller')->as('seller.')->group(function () {
//
//    Route::resource('warehouse', 'WarehouseController');
//    // Vendor user location
//    Route::resource('location', 'LocationController');
...
Route::group(['middleware' => ['auth']], function() {
    ...
    // Switch Account
    Route::get('/switch-account','HomeController@switchAccout')->name('auth.switch');
    Route::get('seller-create','HomeController@sellerCreate')->name('become.seller');

    // Daraz Route
    Route::get('daraz-callback', 'DarazController@darazCallback')->name('callback.daraz');
    Route::post('/update-daraz-settings','DarazController@updateSettings')->name('daraz_setting.update');
    Route::get('/disconnect-daraz', 'DarazController@disconnectAccount')->name('disconnect.daraz');

    // Package Route
    Route::get('purchase-info/{id}','CustomerPackageController@getPackageInfo')->name('purchase_info.package');
    Route::get('renew-package','CustomerPackageController@renewPackage')->name('renew.package');
    Route::post('renew-package','CustomerPackageController@renewPackageUpdate')->name('renew.package');

    // Daraz Shop
    Route::resource('seller-shop', 'SellerShopController');
    Route::post('verify-shop','SellerShopController@verifyShop')->name('seller-shop.verify');
    Route::post('re-verify-shop','SellerShopController@reVerifyShop')->name('seller-shop.re-verify');
    Route::get('seller-shop-active/{slug}','SellerShopController@activeShop')->name('seller-shop.active');
});
